...
xiv.ViewBox = function (opt_args) {
    ...
    this._element.appendChild(this.SlicerViewMenu_._element);
    ...
    this.doNotHide(this.SlicerViewMenu_._element);
...
xiv.ViewBox.prototype.loadThumbnail = function (thumb) {
    ...
    case 'slicer':
	this.SlicerViewMenu_._element.style.visibility = 'visible';
	this.Displayer_.loadSlicer(thumb.xnatProperties.files);
	//return;
	break;
    default:
	this.SlicerViewMenu_._element.style.visibility = 'hidden';
...
xiv.ViewBox.prototype.onContentDividerDragged_ = function() {	
    ...
    //
    if (utils.style.dims(this.ContentDivider_._element, 'top') < this.ContentDivider_.getLowerLimit()) {
