...
SliderLinker.prototype.clearSelectorBox = function(ViewBox) {
	...
	//("CLEARING SELECTOR BOX of ", xiv.ViewBox.element.id)
	xiv.ViewBox.selectorBox.style.border = "none";		
	
	goog.events.unlisten(xiv.ViewBox.element, goog.events.EventType.MOUSEOVER, that.sliderlink_mouseover);
	goog.events.unlisten(xiv.ViewBox.element, goog.events.EventType.MOUSEOUT, that.sliderlink_mouseout);
	
	goog.array.forEach(xiv.ViewBox.element.defaultMouseEvents, function(event) {
		event();		
	})
...
SliderLinker.prototype.processGroups = function () {
	...
	that.forEach_(function(viewer) {
		...
		goog.events.listen(viewer.element, goog.events.EventType.MOUSEOVER, that.sliderlink_mouseover);
		goog.events.listen(viewer.element, goog.events.EventType.MOUSEOUT, that.sliderlink_mouseout);
	...
	XV.xiv.ViewBoxManager( function(ViewBox) {
		...
		if (!xiv.ViewBox.selectorBox) {
			...
			xiv.ViewBox.selectorBox =  xiv.SliderLinker.addSelectorBox( 
				xiv.ViewBox.element.parentNode , 
				utils.style.dims(xiv.ViewBox.element)
			);
