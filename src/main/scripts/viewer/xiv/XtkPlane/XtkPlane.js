...
xiv.XtkPlane = function(id) {
    ...
    goog.dom.classes.set(this._element, xiv.XtkPlane.ELEMENT_CLASS);
	...
	// Slider
	//
	this.Slider_ = new utils.ui.GenericSlider({'parent': this._element, 'orientation' : 'horizontal'});
...
xiv.XtkPlane.prototype.updateStyle = function (opt_args) {
    ...
    //------------------
    if (this.Slider_) {
	if (this._element.parentNode && this.Slider_._element.parentNode !== this._element.parentNode) {
	    this._element.parentNode.appendChild(this.Slider_._element);
	}
