...
xiv.ViewBoxTabs.prototype.reset = function(opt_tabtitle) {
	...
	// Remove tab from dom.
	//
	goog.array.forEach(goog.dom.getElementsByClass(xiv.ViewBoxTabs.TAB_CLASS, this._element), function(tab, i) { 
	    goog.dom.removeNode(tab);
	    delete tab;
	})
	// Remove tab page from dom.
	goog.array.forEach(goog.dom.getElementsByClass(xiv.ViewBoxTabs.TABPAGE_CLASS, this._element), function(tab, i) { 
	    goog.dom.removeNode(tab);
	    delete tab;
	})
	...
	//
	goog.array.forEach(goog.dom.getElementsByClass(xiv.ViewBoxTabs.TABICON_CLASS, this._element), function(tab, i) { 
	    goog.dom.removeNode(tab);
	    delete tab;
	})
    }
...
xiv.ViewBoxTabs.prototype.addTab = function(tabTitle) {
    ...
    //  Tab page
    //------------------	
    var tabPage = utils.dom.makeElement('div', this._element.parentNode, xiv.ViewBoxTabs.TABPAGE_CLASS);
...
xiv.ViewBoxTabs.prototype.getTabPage = function (value) {
	...
	goog.array.forEach(goog.dom.getElementsByClass(xiv.ViewBoxTabs.TABPAGE_CLASS, that._element), function(tabPage, i) { 			
...
xiv.ViewBoxTabs.prototype.setActive = function (activeTabNum) {	
    ...
    //------------------
    goog.array.forEach(goog.dom.getElementsByClass(xiv.ViewBoxTabs.TAB_CLASS, that._element), function(tab, i) { 
	if (i === activeTabNum) {
    ...
    //------------------
    goog.array.forEach(goog.dom.getElementsByClass(xiv.ViewBoxTabs.TABPAGE_CLASS, that._element), function(tabPage) {
	if (activeTabNum > -1){
...
xiv.ViewBoxTabs.prototype.updateStyle = function (opt_args) {
    ...
    //  the class.
    //------------------
    goog.dom.classes.add(this._element, xiv.ViewBoxTabs.ELEMENT_CLASS);
    ...
    //------------------
    if (goog.dom.getElementsByClass(xiv.ViewBoxTabs.TAB_CLASS, this._element).length > 0) {
	...
	goog.array.forEach(goog.dom.getElementsByClass(xiv.ViewBoxTabs.TABPAGE_CLASS, that._element), function(elt, i) {			
	...
	//
	var tabs = goog.dom.getElementsByClass(xiv.ViewBoxTabs.TAB_CLASS, this._element);
